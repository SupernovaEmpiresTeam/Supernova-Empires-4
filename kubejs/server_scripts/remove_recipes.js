
// Remove Recipes by id 

onEvent('recipes', event => {

    // Minecraft

    event.remove({id: 'minecraft:netherite_helmet_smithing'})
    event.remove({id: 'minecraft:netherite_chestplate_smithing'})
    event.remove({id: 'minecraft:netherite_leggings_smithing'})
    event.remove({id: 'minecraft:netherite_boots_smithing'})
    event.remove({id: 'minecraft:diamond_helmet'})
    event.remove({id: 'minecraft:diamond_chestplate'})
    event.remove({id: 'minecraft:diamond_leggings'})
    event.remove({id: 'minecraft:diamond_boots'})
    event.remove({id: 'minecraft:diamond_pickaxe'})
    event.remove({id: 'minecraft:diamond_axe'})
    event.remove({id: 'minecraft:diamond_sword'})
    event.remove({id: 'minecraft:diamond_shovel'})
    event.remove({id: 'minecraft:diamond_hoe'})

    // Extended Crafting

    event.remove({id: 'extendedcrafting:basic_table'})
    event.remove({id: 'extendedcrafting:advanced_table'})
    event.remove({id: 'extendedcrafting:elite_table'})
    event.remove({id: 'extendedcrafting:ultimate_table'})
    event.remove({id: 'extendedcrafting:basic_auto_table'})
    event.remove({id: 'extendedcrafting:advanced_auto_table'})
    event.remove({id: 'extendedcrafting:elite_auto_table'})
    event.remove({id: 'extendedcrafting:ultimate_auto_table'})
    event.remove({id: 'extendedcrafting:pedestal'})
    event.remove({id: 'extendedcrafting:crafting_core'})
    event.remove({id: 'extendedcrafting:compressor'})
    event.remove({id: 'extendedcrafting:luminessence'})
    event.remove({id: 'extendedcrafting:the_ultimate_catalyst'})
  
    // Mekanism

    event.remove({id: 'mekanism:metallurgic_infuser'})
    event.remove({id: 'mekanism:digital_miner'})
    event.remove({id: 'mekanismadditions:obsidian_tnt'})
    event.remove({id: 'mekanism:hdpe_sheet'})
    event.remove({id: 'mekanism:mekasuit_helmet'})
    event.remove({id: 'mekanism:mekasuit_bodyarmor'})
    event.remove({id: 'mekanism:mekasuit_pants'})
    event.remove({id: 'mekanism:mekasuit_boots'})
    event.remove({id: 'mekanism:robit'})
    event.remove({id: 'mekanism:network_reader'})
    event.remove({id: 'mekanism:portable_teleporter'})
    event.remove({id: 'mekanism:seismic_reader'})
    event.remove({id: 'mekanism:atomic_disassembler'})
    event.remove({id: 'mekanism:electric_bow'})
    event.remove({id: 'mekanism:free_runners'})
    event.remove({id: 'mekanism:induction/cell/basic'})
    event.remove({id: 'mekanism:induction/cell/advanced'})
    event.remove({id: 'mekanism:induction/cell/elite'})
    event.remove({id: 'mekanism:induction/cell/ultimate'})
    event.remove({id: 'mekanism:chargepad'})
    event.remove({id: 'mekanism:laser'})
    event.remove({id: 'mekanism:resistive_heater'})
    event.remove({id: 'mekanism:energy_cube/basic'})
    event.remove({id: 'mekanism:energy_cube/advanced'})
    event.remove({id: 'mekanism:energy_cube/elite'})
    event.remove({id: 'mekanism:energy_cube/ultimate'})
    event.remove({id: 'mekanismgenerators:generator/wind'})
    event.remove({id: 'mekanism:enrichment_chamber'})
    event.remove({id: 'mekanism:osmium_compressor'})
    event.remove({id: 'mekanism:combiner'})
    event.remove({id: 'mekanism:crusher'})
    event.remove({id: 'mekanism:purification_chamber'})
    event.remove({id: 'mekanism:energized_smelter'})
    event.remove({id: 'mekanism:chemical_oxidizer'})
    event.remove({id: 'mekanism:chemical_infuser'})
    event.remove({id: 'mekanism:chemical_injection_chamber'})
    event.remove({id: 'mekanism:electrolytic_separator'})
    event.remove({id: 'mekanism:precision_sawmill'})
    event.remove({id: 'mekanism:chemical_dissolution_chamber'})
    event.remove({id: 'mekanism:chemical_crystallizer'})
    event.remove({id: 'mekanism:seismic_vibrator'})
    event.remove({id: 'mekanism:pressurized_reaction_chamber'})
    event.remove({id: 'mekanismgenerators:generator/gas_burning'})


    // Thermal

    event.remove({id: 'thermal:parts/iron_gear'})
    event.remove({id: 'thermal:parts/gold_gear'})
    event.remove({id: 'thermal:parts/netherite_gear'})
    event.remove({id: 'thermal:parts/diamond_gear'})
    event.remove({id: 'thermal:parts/lapis_gear'}) // no other gear yet
    event.remove({id: 'thermal:parts/emerald_gear'}) // no other gear yet
    event.remove({id: 'thermal:parts/quartz_gear'}) // no other gear yet
    event.remove({id: 'thermal:parts/electrum_gear'})
    event.remove({id: 'thermal:parts/invar_gear'}) // no other grear yet
    event.remove({id: 'thermal:parts/signalum_gear'}) // no other gear yet
    event.remove({id: 'thermal:parts/lumium_gear'}) // no other gear yet
    event.remove({id: 'thermal:parts/enderium_gear'}) // no other gear yet
    event.remove({id: 'thermal:parts/iron_gear'})

    // ProjectE

    event.remove({id: 'projecte:repair_talisman'})
    event.remove({id: 'projecte:philosophers_stone'})
    event.remove({id: 'projecte:philosophers_stone_alt'})

    // Applied Energistics 2

    event.remove({id: 'lazierae2:etcher'})
    event.remove({id: 'lazierae2:energizer'})
    event.remove({id: 'lazierae2:centrifuge'})
    event.remove({id: 'lazierae2:aggregator'})

    // Powah

    event.remove({id: 'powah:crafting/reactor_starter'})
    event.remove({id: 'powah:crafting/reactor_basic'})
    event.remove({id: 'powah:crafting/reactor_hardened'})
    event.remove({id: 'powah:crafting/reactor_blazing'})
    event.remove({id: 'powah:crafting/reactor_niotic'})
    event.remove({id: 'powah:crafting/reactor_spirited'})
    event.remove({id: 'powah:crafting/reactor_nitro'})

    // Draconic Evolution

    event.remove({id: 'draconicevolution:modules/chaotic_aoe'})
    event.remove({id: 'draconicevolution:modules/chaotic_damage'})
    event.remove({id: 'draconicevolution:modules/chaotic_energy'})
    event.remove({id: 'draconicevolution:modules/chaotic_flight'})
    event.remove({id: 'draconicevolution:modules/chaotic_jump'})
    event.remove({id: 'draconicevolution:modules/chaotic_large_shield_capacity'})
    event.remove({id: 'draconicevolution:modules/chaotic_proj_accuracy'})
    event.remove({id: 'draconicevolution:modules/chaotic_proj_damage'})
    event.remove({id: 'draconicevolution:modules/chaotic_proj_grav_comp'})
    event.remove({id: 'draconicevolution:modules/chaotic_proj_penetration'})
    event.remove({id: 'draconicevolution:modules/chaotic_proj_velocity'})
    event.remove({id: 'draconicevolution:modules/chaotic_shield_capacity'})
    event.remove({id: 'draconicevolution:modules/chaotic_shield_control'})
    event.remove({id: 'draconicevolution:modules/chaotic_shield_recovery'})
    event.remove({id: 'draconicevolution:modules/chaotic_speed'})
    event.remove({id: 'draconicevolution:modules/chaotic_undying'})
    event.remove({id: 'draconicevolution:modules/draconic_aoe'})
    event.remove({id: 'draconicevolution:modules/draconic_auto_feed'})
    event.remove({id: 'draconicevolution:modules/draconic_damage'})
    event.remove({id: 'draconicevolution:modules/draconic_energy'})
    event.remove({id: 'draconicevolution:modules/draconic_flight'})
    event.remove({id: 'draconicevolution:modules/draconic_jump'})
    event.remove({id: 'draconicevolution:modules/draconic_large_shield_capacity'})
    event.remove({id: 'draconicevolution:modules/draconic_proj_accuracy'})
    event.remove({id: 'draconicevolution:modules/draconic_proj_anti_immune'})
    event.remove({id: 'draconicevolution:modules/draconic_proj_damage'})
    event.remove({id: 'draconicevolution:modules/draconic_proj_grav_comp'})
    event.remove({id: 'draconicevolution:modules/draconic_proj_penetration'})
    event.remove({id: 'draconicevolution:modules/draconic_proj_velocity'})
    event.remove({id: 'draconicevolution:modules/draconic_shield_capacity'})
    event.remove({id: 'draconicevolution:modules/draconic_shield_control'})
    event.remove({id: 'draconicevolution:modules/draconic_shield_recovery'})
    event.remove({id: 'draconicevolution:modules/draconic_speed'})
    event.remove({id: 'draconicevolution:modules/draconic_undying'})
    event.remove({id: 'draconicevolution:modules/draconium_aoe'})
    event.remove({id: 'draconicevolution:modules/draconium_auto_feed'})
    event.remove({id: 'draconicevolution:modules/draconium_damage'})
    event.remove({id: 'draconicevolution:modules/draconium_energy'})
    event.remove({id: 'draconicevolution:modules/draconium_jump'})
    event.remove({id: 'draconicevolution:modules/draconium_speed'})
    event.remove({id: 'draconicevolution:modules/wyvern_aoe'})
    event.remove({id: 'draconicevolution:modules/wyvern_auto_feed'})
    event.remove({id: 'draconicevolution:modules/wyvern_auto_fire'})
    event.remove({id: 'draconicevolution:modules/wyvern_damage'})
    event.remove({id: 'draconicevolution:modules/wyvern_energy'})
    event.remove({id: 'draconicevolution:modules/wyvern_flight'})
    event.remove({id: 'draconicevolution:modules/wyvern_hill_step'})
    event.remove({id: 'draconicevolution:modules/wyvern_jump'})
    event.remove({id: 'draconicevolution:modules/wyvern_large_shield_capacity'})
    event.remove({id: 'draconicevolution:modules/wyvern_proj_accuracy'})
    event.remove({id: 'draconicevolution:modules/wyvern_proj_damage'})
    event.remove({id: 'draconicevolution:modules/wyvern_proj_grav_comp'})
    event.remove({id: 'draconicevolution:modules/wyvern_proj_penetration'})
    event.remove({id: 'draconicevolution:modules/wyvern_proj_velocity'})
    event.remove({id: 'draconicevolution:modules/wyvern_shield_capacity'})
    event.remove({id: 'draconicevolution:modules/wyvern_shield_control'})
    event.remove({id: 'draconicevolution:modules/wyvern_shield_recovery'})
    event.remove({id: 'draconicevolution:modules/wyvern_speed'})
    event.remove({id: 'draconicevolution:modules/wyvern_undying'})
    
  
})





onEvent('item.registry', event => {

    // Ingots  to unify

    event.create('steel_ingot').displayName('Steel Ingot')
    event.create('dark_steel_ingot').displayName('Dark Steel Ingot')
    event.create('draconium_ingot').displayName('Draconium Ingot')
    event.create('awakened_draconium_ingot').displayName('Awakened Draconium Ingot')
    event.create('copper_ingot').displayName('Copper Ingot')
    event.create('bronze_ingot').displayName('Bronze Ingot')
    event.create('tin_ingot').displayName('Tin Ingot')
    event.create('nickel_ingot').displayName('Nickel Ingot')
    event.create('constantan_ingot').displayName('Constantan Ingot')
    event.create('lead_ingot').displayName('Lead Ingot')
    event.create('silver_ingot').displayName('Silver Ingot')
    event.create('electrum_ingot').displayName('Electrum Ingot')
    event.create('uranium_ingot').displayName('Uranium Ingot')
    event.create('osmium_ingot').displayName('Osmium Ingot')
    event.create('aluminium_ingot').displayName('Aluminium Ingot')

    // Ported from contenttweaker

    event.create('fbmodium_ingot').displayName('Fbmodium Ingot')
    event.create('nether_infused_fbmodium_ingot').displayName('Nether Infused Fbmodium Ingot')
    event.create('end_infused_fbmodium_ingot').displayName('End Infused Fbmodium Ingot')
    event.create('ultimate_infused_fbmodium_ingot').displayName('Ultimate Infused Fbmodium Ingot')
    event.create('blue_iron_ingot').displayName('Blue Iron Ingot')
    event.create('bluestone_ingot').displayName('Bluestone Ingot Ingot')
    event.create('light_bluestone_ingot').displayName('Light Bluestone Ingot')
    event.create('greenstone_ingot').displayName('Greenstone Ingot')

})
